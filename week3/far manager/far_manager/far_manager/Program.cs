...
{
    enum FSIMode // Объявление перечисления происходит с помощью оператора enum
    {            
        Folder = 0, //каждому элементу перечисления присваивается целочисленное значение, причем первый элемент будет иметь значение 0, второй - 1 и так далее.
        File = 1    
    }
    class Layer // создание класса
    {           //класс представляет новый тип, который определяется пользователем
        int selectedIndex;
        public DirectoryInfo[] Directories // свойство для папок
        {
            get; //Стандартное определение свойства содержит блоки get и set. 
            set; //В блоке get мы возвращаем значение поля, а в блоке set устанавливаем.
        }
        public FileInfo[] Files // свойство для файлов
        {
            get;
            set;
        }
        public int SelectedIndex //свойство для курсора
        {
            get
            {
            ...
            set
            {
                ...
                else selectedIndex = value;   // для того чтобы курсор не уходил за пределы списка
            }
        }
        void SelectedColor(int i)
        {
            if (i == SelectedIndex)
                Console.BackgroundColor = ConsoleColor.Green; // цвет курсора
            else
        ...
        public void Draw()
        {
            Console.BackgroundColor = ConsoleColor.Blue; // цвет фона
            Console.Clear();
            Console.ForegroundColor = ConsoleColor.Red; // цвет названия папок
            for (int i = 0; i < Directories.Length; i++)
            {
                SelectedColor(i);
                Console.WriteLine((i + 1) + ". " + Directories[i].Name); // пронумерование элементов директории
            }
            Console.ForegroundColor = ConsoleColor.Yellow; // цвет файлов внутри папок
            for (int i = 0; i < Files.Length; i++)
            {
                SelectedColor(i + Directories.Length);
                Console.WriteLine((i + Directories.Length + 1) + ". " + Files[i].Name); // пронумерование файлов
            }
        }
    }
    class Program
    {
        ...
        {
            DirectoryInfo di = new DirectoryInfo(@"C:\Users\Akimgereyev\Desktop\whotag"); // путь до папки
            Layer l = new Layer // новый экземпляр класса layer
            {
                Directories = di.GetDirectories(),
                Files = di.GetFiles(),
                SelectedIndex = 0           }; 
            Stack<Layer> history = new Stack<Layer>();// для перемещения между папками и файлами
            history.Push(l);
            bool x = false;
            ...
            while (!x)
            {
                ...
                ConsoleKeyInfo ifkey = Console.ReadKey(); // обьявление новой структуры для чтения клавиш
